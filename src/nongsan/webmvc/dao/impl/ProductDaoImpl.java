...
public class ProductDaoImpl extends connectDB implements ProductDao {
    ...
    @Override
    public List<Product> getProductByTranID(int id) {

        List<Product> productList = new ArrayList<Product>();
        String sql = "SELECT * FROM dbo.product,dbo.ordered WHERE dbo.ordered.product_id = dbo.product.id AND dbo.ordered.transaction_id = 1 ? ";
        Connection conn = connectDB.getConnect();
